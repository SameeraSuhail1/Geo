...
find_metro_number <- function(codes,file2) {
  
  codes$code_area <- st_area(codes)
  int2 <- as_tibble(st_intersection(codes,file2))
  int2$num <- 1
  metrobyCode <- aggregate(list(num=int2$num,code_area=int2$code_area), by=list(int2$mccid_int),FUN=sum, na.rm =TRUE)
  
  #PGbyCode$numpersqm = PGbyCode$num/PGbyCode$code_area
  colnames(metrobyCode)[colnames(metrobyCode)=="Group.1"] <- "mccid_int"
  
  final_file <- merge(codes,metrobyCode,by="mccid_int",, all.x=TRUE)
  final_file[is.na(final_file)] <- 0
  final_file$code_area.y =NULL
  colnames(final_file)[colnames(final_file)=="code_area.x"] <- "code_area"
  return(final_file)
...
find_bar_number <- function(codes,file2) {
  
  codes$code_area <- st_area(codes)
  int2 <- as_tibble(st_intersection(codes,file2))
  int2$num <- 1
  BarbyCode <- aggregate(list(num=int2$num,code_area=int2$code_area), by=list(int2$mccid_int),FUN=sum, na.rm =TRUE)
  
  #PGbyCode$numpersqm = PGbyCode$num/PGbyCode$code_area
  colnames(BarbyCode)[colnames(BarbyCode)=="Group.1"] <- "mccid_int"
  
  final_file <- merge(codes,BarbyCode,by="mccid_int",, all.x=TRUE)
  final_file[is.na(final_file)] <- 0
  final_file$code_area.y =NULL
  colnames(final_file)[colnames(final_file)=="code_area.x"] <- "code_area"
  return(final_file)
...
find_nearest_distance <- function(origAddress, codes,file,col_name) {
  ...
  file_code<-st_intersection(codes,file)
  
  
  #for each feature (geometry) in house_in_codes, it returns the index of the nearest feature (geometry) in file_code
  temp<- as.vector(st_nearest_feature(house_in_codes, file_code))
  
  file_code$rownumber = 1:nrow(file_code)
  
  nearest_file <- file_code[temp,]
  
  house_file_distance<-as.data.frame(st_distance(house_in_codes,nearest_file, by_element = TRUE))
  colnames(house_file_distance) <- col_name
  
  house_table <- cbind(as.data.frame(house_in_codes),house_file_distance, as.data.frame(nearest_file))
